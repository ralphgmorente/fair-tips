...
- For each store sale, active staff are employees with a valid shift where `clock-in <= order date/time <= clock-out`.
- If active staff exist, the store order tip is split evenly across those active employees.
- If no active staff exist, the store order tip is unallocated and appears for review.
